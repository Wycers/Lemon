...
v-container
        ...
        tr
          ...
          td(v-if='actions !== false', width='160')
            template(v-for="(value, action) in actions")
              v-btn(v-if="['edit', 'delete'].indexOf(action) < 0", router,primary,fab,small,dark,:to="{name: action, params: {resource,id:props.item.uid}}")
                v-icon {{action.icon ? action.icon : action}}
            v-btn(v-if="options.edit !== false",dark,primary,fab,small,:to="{name: 'edit', params: {resource, id:props.item.uid}}")
              v-icon edit
  ...
  v-dialog(v-model="isShowEdit", width="70%")
    v-card
      ...
      v-card-text
        v-form(v-model="form.model", v-bind="form", method="patch", :action="resource+'/'+currentItem.uid", @success="onSaveEdit")
